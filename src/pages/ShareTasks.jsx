...
export default function ShareTasks() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-5">
      <h1 className="text-3xl font-bold mb-2">Share Tasks</h1>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {tasks
            ...
            .map((task) => {
              ...
              const current = progress?.messageCount || 0;

              const total = task.requiredMessages || 1;
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    <div className="mt-5 space-y-2 text-sm">
                      <div>
                        💰 Reward:
                        <strong> {task.rewardCoins} Coins</strong>
                      </div>

                      <div>
                        👥 Required Users:
                        <strong> {task.requiredMessages}</strong>
                      </div>

                      {task.allowedTypes?.length > 0 && (
                        <div>
                          📩 Allowed:
                          <strong> {task.allowedTypes.join(", ")}</strong>
                        </div>
                      )}
                      ...
                      {task.requiredKeyword && (
                        <div>
                          🔑 Keyword:
                          <strong> {task.requiredKeyword}</strong>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="mt-6 flex gap-3">
                      ...
                      {task.image && (
                        <button
                          onClick={() => handleShare(task, "image")}
