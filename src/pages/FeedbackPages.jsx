...
export default function FeedbackPages() {
    ...
    const userFeedbacks = feedbacks.filter((fb) => fb.user === user?._id);

    if (userFeedbacks.length === 0) {
        return <p className="text-center">You have not created any feedback yet.</p>;
    }
            ...
            {userFeedbacks.map((fb) => (
                <div key={fb._id} className="bg-white p-4 rounded shadow">
                    ...
                    <p className="text-sm mb-2">
                        Status: <span className={`capitalize font-semibold ${fb.status === 'open' ? 'text-green-600' : fb.status === 'pending' ? 'text-yellow-600' : 'text-red-600'}`}>
                            {fb.status}
                        </span>
                    </p>
