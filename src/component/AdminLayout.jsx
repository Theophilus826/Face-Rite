...
export default function AdminLayout() {
  ...
  /* =========================================================
     ...
     Pure event-driven, no gamesList
  ========================================================= */
  useEffect(() => {
    ...
    const handleEvent = (event) => {
      ...
      setGames((prevGames) => {
        switch (event.type) {
          case "GAME_CREATED":
            if (prevGames.some((g) => g.gameId === event.gameId)) return prevGames;
            return [
              {
                gameId: event.gameId,
                hostId: event.hostId,
          ...
          case "GAME_DELETED":
            return prevGames.filter((g) => g.gameId !== event.gameId);
  ...
  return (
    <>
      ...
      {/* LIVE GAME CONTROLLER (ONLY SHOW WHEN HOST EXISTS) */}
      <section className="mt-4 bg-white p-4 shadow rounded">
        ...
        <div className="max-h-[400px] overflow-y-auto">
          {games
            .filter((game) => game.hostId) // <-- only show games with a host
            .map((game) => (
              <div
                key={game.gameId}
              ...
              >
                ...
                <div className="text-sm text-gray-500">Host: {game.hostId}</div>
